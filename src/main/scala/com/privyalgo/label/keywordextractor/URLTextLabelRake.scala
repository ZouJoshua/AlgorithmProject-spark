...
class URLTextLabelRake (
  ...
  def run(): /*DataFrame*/RDD[(String,List[String])] = {
    ...
    val res = text.map(line => {
      ...
      val pnStr = strJson.get("url").toString
      ...
      if (count != 3 && strJson.containsKey("real_lang")) {
        ...
        if ("en".equals(realLang)) {
          ...
          val descInfo = sb.toString()
          val infoWords = getTextLabel(descInfo, bcStopWords).take(100)
