...
object NewsSubCategoryTrainCheck {
  def main(args: Array[String]): Unit = {
    ...
    def entertainment_update_check(spark: SparkSession) = {
      ...
      val drop1 = df.filter("two_level = predict_two_level").filter("predict_two_level_proba < 0.4").filter("two_level != 'others'").select("article_id")
      val drop2 = df.filter("two_level != predict_two_level").filter("predict_two_level_proba > 0.7").select("article_id")
      val drop = drop1.union(drop2).withColumn("drop",lit(1))
      val ori_df = spark.read.json("news_content/sub_classification/entertainment/entertainment_update")
      val new_ori = ori_df.join(drop, Seq("article_id"), "left").filter("drop is null").drop("drop")
      new_ori.coalesce(1).write.format("json").mode("overwrite").save("news_content/sub_classification/entertainment/entertainment_update_v1")
    }
...
//      result.coalesce(1).write.format("json").mode("overwrite").save("news_content/sub_classification/business/business_update_tmp_v1")
    ...
    def business_update_check_v1(spark: SparkSession) = {
      val df = spark.read.json("news_content/sub_classification_check/business_update*")
      val drop1 = df.filter("two_level = predict_two_level").filter("predict_two_level_proba < 0.4").filter("two_level in ('economy','stock&bond','market','invest')").select("article_id")
      val drop2 = df.filter("two_level != predict_two_level").filter("predict_two_level_proba > 0.6").filter("two_level in ('economy','stock&bond','market','invest','company','industry')").select("article_id")
      val drop = drop1.union(drop2).withColumn("drop", lit(1))
      val ori_df = spark.read.json("news_content/sub_classification/business/business_update")
      val two_level_replace = df.filter("two_level != predict_two_level").filter("predict_two_level_proba > 0.7").selectExpr("article_id","predict_two_level as two_level")
      val ori_replace = ori_df.drop("two_level").join(two_level_replace, Seq("article_id")).select("article_id","url","title","content","one_level","two_level","three_level")
...
//      val df = spark.read.json("news_content/sub_classification_check/lifestyle*")
    ...
    def lifestyle_update_check(spark: SparkSession) = {
      val df = spark.read.json("news_content/sub_classification_check/lifestyle_update*")
//      val df = spark.read.json("news_content/sub_classification_check/tmp/lifestyle*")
      val drop1 = df.filter("two_level = predict_two_level").filter("predict_two_level_proba < 0.4").select("article_id")
      val drop2 = df.filter("two_level != predict_two_level").filter("predict_two_level_proba > 0.65").select("article_id")
      val drop_df = drop1.union(drop2).withColumn("drop", lit(1))
      val ori_df = spark.read.json("news_content/sub_classification/lifestyle/lifestyle_update")
      ori_df.coalesce(1).write.format("json").mode("overwrite").save("news_content/sub_classification/lifestyle/lifestyle_update_v1")
    }
    ...
    def international_update_heck(spark: SparkSession) = {
      val df = spark.read.json("news_content/sub_classification_check/international_update*")
      val drop1 = df.filter("two_level = predict_two_level").filter("predict_two_level_proba < 0.5").filter("two_level in ('politics','society')").select("article_id")
      val drop2 = df.filter("two_level = predict_two_level").filter("predict_two_level_proba < 0.4").filter("two_level not in ('politics','society')").select("article_id")
      val drop3 = df.filter("two_level != predict_two_level").filter("predict_two_level_proba > 0.65").filter("two_level in ('politics','society')").select("article_id")
      val drop_df = drop1.union(drop2).union(drop3).withColumn("drop", lit(1))
      val ori_df = spark.read.json("news_content/sub_classification/international/international_update")
      val new_ori = ori_df.join(drop_df, Seq("article_id"), "left").filter("drop is null").drop("drop")
      new_ori.coalesce(1).write.format("json").mode("overwrite").save("news_content/sub_classification/international/international_update_v1")
    }
