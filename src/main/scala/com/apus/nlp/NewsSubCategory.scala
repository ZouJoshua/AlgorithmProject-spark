...
object NewsSubCategory {
  def main(args: Array[String]): Unit = {
    ...
    def national_data_processer_v3(spark: SparkSession,
      ...
      val politics_df = {
        class_df.filter("two_level = 'politics'")
          .select("article_id", "title", "content", "one_level", "two_level", "three_level").limit(13000)
      }
      val education_df = {
    ...
    def entertainment_data_processer_v1(spark: SparkSession,
      ...
      val class_df = nonull_df.join(ori_df, Seq("article_id"))
      val all_main = {
        class_df.filter("two_level in ('bollywood','celebrity','tv','movie','music','comic','hollywood')")
          .select("article_id", "title", "content", "one_level", "two_level", "three_level")
      }
      val others = {
        class_df.filter("two_level in ('art+culture+history','cartoon&comics','dance', 'others','variety show','performance')")
          .drop("two_level")
