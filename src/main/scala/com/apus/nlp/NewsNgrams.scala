...
object NewsNgrams {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def write_NGramDF(spark:SparkSession,
                      savePath:String="N_gramDF_manually",
                      loadPath:String,stopwords_bd:Broadcast[Map[String,Double]]) = {
      val sc = spark.sparkContext
      import spark.implicits._
      // load inpDF
      val inpDF = {
      ...
      def ngramsWithFastNLP(inpDF:DataFrame):DataFrame = {
          ...
          rs =>
            ...
            rs.map{
              r =>
                ...
                val ngram = Ngrams(lemmas, 1, 4)
                (article_id, text, ngram)
            }
        }.toDF("article_id","article","ngrams").filter("size(ngrams) > 0")
      }

      def Ngrams(inp_seq: Seq[String], minSize: Int = 1, maxSize: Int= 4): Seq[String] = {
      ...
      // 去掉了长度不足3或超过40的 n元词
      def ngramWithManuallySplit(inpDF:DataFrame):DataFrame = {
            ...
            rs.map{
              r =>
                ...
                val title_ngram = {
                  ...
                  val ngram = Ngrams(clean_title.split(" "), 1, 4).filter(_.length>1)
                ...
                val content_ngram = {
                  val ngram = Ngrams(clean_text.split(" ").filter(_.length>1)/*.map(_.toLowerCase)*/, 1, 4).filter(!stopwords_bd.value.contains(_)).filter(x => x.length<=max_str_length && x.length>=min_word_lenth)
                  val certain_words = {
      ...
      //    匹配括号内的内容 |  "(?<=\\()[^)]+(?=\\))"
      def ngramWithManuallySplit_v2(inpDF:DataFrame):DataFrame = {
          ...
          rs =>
            rs.map{
              r =>
                val article_id = r.getAs[String]("article_id")
                val article_title = " "+r.getAs[String]("title")+" "
                val text = " "+r.getAs[String]("article_parsed")+" "

                val title_Ngram_ = {
                  val direct_ngram = Ngrams(article_title.split(" "))
                  val replaced_ngram = {
                    val replaced = {
                      (" "+article_title+" ")
                ...
                val nounLongerOne_InTitle = RecUtil.onlyLongerOne(noun)
                val title_Ngram = nounLongerOne_InTitle++title_Ngram_

                val sentence_list = text.split("\\.(?=\\s)")
                val text_NGram_ = {
                  val direct_ngram = sentence_list.flatMap(s => Ngrams(s.trim.split(" ")))
                ...
                val nounLongerOne_InContent = RecUtil.onlyLongerOne(noun_InContent)
                val text_NGram = nounLongerOne_InContent++text_NGram_

                (article_id,text,text_NGram,title_Ngram,text_NGram++title_Ngram)
            }
        }.toDF("article_id","article_parsed","content_ngram","title_ngram","n_gram").filter("size(n_gram) > 0")
      }
      val N_gramDF_ = ngramWithManuallySplit_v2(inpDF)
      // write
      N_gramDF_.cache
